...
const HIGH_CONFIDENCE = 0.9; // short-circuit threshold — tune once you see real test results
...
async function narrowVariant(productId, brandText, sizeText) {
  ...
  let brandIgnored = false;
  let sizeIgnored = false;
  ...
  if (candidates.length === 1) {
    const result = { outcome: "confident_match", variant: candidates[0] };

    // Flag when brand/size text was given but genuinely doesn't apply to this
    // product (not just "didn't match" — the product has no such distinction
    // at all) — so the AI can acknowledge it honestly instead of silently
    // dropping what the customer said.
    const notes = [];
    if (brandIgnored && candidates[0].brand == null) {
      notes.push("brand_not_applicable");
    }
    if (sizeIgnored && candidates[0].size == null) {
      notes.push("size_not_applicable");
    }
    if (notes.length > 0) {
      result.notes = notes;
    }
...
async function searchProducts(productQuery, brandText, sizeText) {
  const productCandidates = await findProduct(productQuery);
  ...
  if (productCandidates.length > 1) {
    return { outcome: "product_ambiguous", candidates: productCandidates };
  }

  const topProduct = productCandidates[0];
  const variantResult = await narrowVariant(topProduct.id, brandText, sizeText);

  // Guard: narrowVariant can report confident_match just because it ran out of
  // things to filter (single variant, or a null-brand product) — that's not the
  // same as genuine confidence if the PRODUCT match itself was only a weak guess.
  // Only trust confident_match when the product identity itself cleared the bar.
  if (
    variantResult.outcome === "confident_match" &&
    topProduct.score < HIGH_CONFIDENCE
  ) {
    return {
      outcome: "product_ambiguous",
      candidates: [topProduct],
    };
  }
