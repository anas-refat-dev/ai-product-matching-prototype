...
async function searchProducts(productQuery, brandText, sizeText) {
  ...
  if (productCandidates.length > 1) {
    // findProduct itself was ambiguous about WHICH PRODUCT — different from
    // narrowVariant's ambiguity, which is about brand/size of an already-known product
    return { outcome: "product_ambiguous", candidates: productCandidates };
  }

  const productId = productCandidates[0].id;
  return narrowVariant(productId, brandText, sizeText);
}
